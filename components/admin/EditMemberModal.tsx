...
export function EditMemberModal({ member }: { member: any }) {
    ...
    const [formData, setFormData] = useState({
        ...
        permissions: (typeof member.permissions === 'string' && member.permissions.startsWith('[')
            ? JSON.parse(member.permissions)
            : Array.isArray(member.permissions)
                ? member.permissions
                : (member.permissions || "").split(",").filter((p: string) => p.length > 0).map((p: string) => ({ id: p, access: "editor" }))
        ) as { id: string, access: "visitor" | "editor" }[]
    });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (formData.permissions.length === 0) {
            toast.error("Assign at least one frequency protocol");
            return;
        }
        setLoading(true);
        try {
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button 
                    variant="ghost" 
                    size="icon" 
                    className="h-12 w-12 text-white/20 hover:text-white hover:bg-white/5 rounded-2xl transition-all border border-transparent hover:border-white/10 shadow-sm"
                >
            ...
            <DialogContent className="sm:max-w-[850px] bg-[#0A0A0A] border-white/5 shadow-[0_40px_150px_rgba(0,0,0,1)] rounded-[48px] p-0 overflow-hidden max-h-[92vh] flex flex-col outline-none">
                <div className="absolute top-0 right-0 w-96 h-96 bg-emerald-600/5 blur-[120px] pointer-events-none -translate-y-1/2 translate-x-1/2" />
                ...
                <div className="relative p-12 pb-6 flex items-center justify-between z-10">
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            <DialogTitle className="text-3xl font-black text-white uppercase tracking-tighter italic leading-none">Modify Protocol</DialogTitle>
                            <p className="text-white/20 text-[10px] font-black uppercase tracking-[0.4em] mt-3">Identity Re-synthesis & Authority Redistribution</p>
                        </div>
                    </div>
                ...
                <div className="overflow-y-auto no-scrollbar px-12 py-10 space-y-12 flex-1 relative z-10">
                    <form id={`edit-member-form-${member.id}`} onSubmit={handleSubmit} className="space-y-16">
                        ...
                        <div className="space-y-8">
                            <div className="flex items-center gap-4">
                                ...
                                <h3 className="text-[11px] font-black uppercase tracking-[0.3em] text-white/40">Profile Update</h3>
                            </div>
                            ...
                            <div className="space-y-4 max-w-md">
                                <Label className="text-[10px] font-black text-white/30 uppercase tracking-widest ml-2">Secured Logical Passphrase (Optional Update)</Label>
                ...
                <div className="p-12 border-t border-white/5 flex gap-6 bg-[#050505]/80 backdrop-blur-3xl shrink-0 z-20">
                    ...
                    >
                        {loading ? "Synchronizing..." : "Synchronize Entity State"}
                    </Button>
                </div>
            </DialogContent>
        </Dialog>
